...
namespace Kinare.FormSystem.Tests
{
    public abstract class FormSystemTestBase : AbpIntegratedTestBase<FormSystemTestModule>
    {
        ...
        protected void UsingDbContext(Action<FormSystemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FormSystemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FormSystemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FormSystemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FormSystemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FormSystemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FormSystemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FormSystemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FormSystemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FormSystemDbContext, Task<T>> func)
        {
            T result;
